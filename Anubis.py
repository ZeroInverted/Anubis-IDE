...
# Importing the new coloring file for C#.
...
text = QTextEdit
text2 = QTextEdit
# Adding the programming language as a global variable in order to use it for checks.
# Making the defualt value as Python since the main functionality of the IDE is compiling Python code.
progLang = "Py"
...
class Widget(QWidget):
    # Added ui as a new parameter to be able to change the toolbar.
    ...
    @pyqtSlot(str)
    def Saving(s):
        # If condition to check the programming language and save the file with the correct file extension.
        if progLang == "Py":
    ...
    def on_clicked(self, index):
        ...
        nn = tuple([nn])
        # Extracting the extension string of the file to know its type.
        fileTypeStr=nn[0].split(".")[1]
        # Checking if the file type is CSharp, then the function that colors and highlights Csharp is called. Otherwise the Python function is called.
        if fileTypeStr == "cs":
            UI.cs_highlighter(self.ui)
        else:
            UI.py_highlighter(self.ui)
...
class UI(QMainWindow):
    ...
    def intUI(self):
        ...
        # After updating the code with extra functionality, we now have four menu items
        filemenu = menu.addMenu('File')
        Port = menu.addMenu('Port')
        # Creating a new variable in the UI class and linking it to the "Prog. Language" tab added in the toolbar menu.
        UI.progLang = menu.addMenu('Prog. Language')
        ...
        # Creating an action for C#, giving it functionality and adding it to the UI.
        cs_action = QAction('C#', self)
        cs_action.triggered.connect(self.cs_highlighter)
        UI.progLang.addAction(cs_action)
        # Creating an action for Python, giving it functionality and adding it to the UI.
        py_action = QAction('Python', self)
        py_action.triggered.connect(self.py_highlighter)
        UI.progLang.addAction(py_action)
        ...
        # Parametrization of the UI so all the changes to the toolbar could take place as it gets sent to the __init__ method.
        widget = Widget(self)
    ...
    def open(self):
        ...
        # File type extension extraction:
        fileTypeStr = self[0].split(".")[1]
        # Calling the file type comparator which will call the appropriate coloring method depending on the file type.
        self.fileType_determiner(fileTypeStr)\
        # If condition to determine which language's highlighter to call.
        if self == "cs":
            UI.cs_highlighter()
        elif self == "py":
            UI.py_highlighter()
    ...
    # CS coloring method that takes the UI, declares and equates the programming language function to Csharp and calls the CSharpHighlighter from CSharp_Coloring.py.
    def cs_highlighter(self):
        global progLang
        progLang = "cs"
        CSharp_Coloring.CSharpHighlighter(text)

    # Python coloring method that takes the UI, declares and equates the programming language function to Python and calls the PythonHighLighter from Python_Coloring.py.
    def py_highlighter(self):
        global progLang
        progLang = "Py"
        Python_Coloring.PythonHighlighter(text)
